...
export default function Filter({onStatusChange,data,DefaultStatus=null,bg=null}) {
    ...
    const [activeStatus, setActiveStatus] = useState(selectedItem.status);  
    const [isHoveringList, setIsHoveringList] = useState(false); 
    const filterRef = useRef();

    // Sync when DefaultStatus changes

    useEffect(() => {
        const item =
            data.find(item => item.status === DefaultStatus) || data[0];
  ...
  return (
         <div ref={filterRef} className={`relative w-full cursor-pointer  `} onClick={()=>setisFilteropened(!isFilteropened)}>

                <ChevronDown className='absolute w-4 h-4 text-gray-500 flex justify-self-end top-3 right-5'/>
                <div  className={`flex items-center justify-start h-10 bg-none outline-none focus:outline-none  ${`${isFilteropened ? 'border-2 border-blue-500 duration-50 ease-in-out':'border border-gray-400/20'} ${bg==='white' && 'bg-white'} `} rounded-xl w-full py-2 px-3  text-sm text-gray-800`}>
